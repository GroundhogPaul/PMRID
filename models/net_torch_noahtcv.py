...
class ResidualBlock(nn.Module):
    ...
    def __init__(self, num_filters):
        ...
        self.conv1 = nn.Conv2d(num_filters, num_filters, kernel_size=3, stride=1, padding=1, padding_mode='zeros')
        self.conv2 = nn.Conv2d(num_filters, num_filters, kernel_size=3, stride=1, padding=1, padding_mode='zeros')
        self.relu = nn.PReLU()
...
class NOAHTCV_Level3(nn.Module):
    """
    Denoising U-Net with simplified architecture using residual blocks.
    """
    def __init__(self, num_filters=32):
        super(NOAHTCV_Level3, self).__init__()
        self.num_filters = num_filters
...
if __name__ == '__main__':
    ...
    net = NOAHTCV_Level3().to(device)

    img, var = torch.randn(1, 4, H, W, device=device, dtype=torch.float32), torch.randn(1, 4, H, W, device=device, dtype=torch.float32)
